...
type JumpCloudPasswordManagerEvent struct {
    ...
    InitiatedBy        struct {
        ...
        Type     string `json:"type"`
        Email    string `json:"email"`
        Username string `json:"username,omitempty"` // Optional, as not all events may have it
    } `json:"initiated_by"`
    Geoip struct {
        CountryCode   string  `json:"country_code"`
        Timezone      string  `json:"timezone"`
        ...
        RegionName    string  `json:"region_name"`
        Longitude     float64 `json:"longitude"`
        RegionCode    string  `json:"region_code,omitempty"` // Optional, may not be present in all logs
    } `json:"geoip"`
    EventType       string `json:"event_type"`
    Resource        struct {
        Name string `json:"name"`
